...
export default function Commissions() {
  ...
  return (
    <Large>
      ...
      <Row
        style={{ justifyContent: "spaceBetween", padding: "0 20px 20px 20px" }}
      >
        <InputWrapper>
          <InputTitle>Тип АК</InputTitle>
          <InputAnchor
            value={
              Object.keys(state).length !== 0 ? state.agencyComissionType : ""
            }
            onChange={(e) =>
              dispatch(
                sendContragentValues("agencyComissionType", e.target.value)
              )
            }
        ...
        <InputWrapper>
          ...
          <InputAnchor
            ...
            onChange={(e) =>
              dispatch(sendContragentValues("agencyComission", e.target.value))
            }
            placeholder="10"
          />
        </InputWrapper>
        <InputWrapper>
          ...
          <InputAnchor
            value={
              Object.keys(state).length !== 0 ? state.agencyDistributed : ""
            }
            onChange={(e) =>
              dispatch(
                sendContragentValues("agencyDistributed", e.target.value)
              )
            }
            placeholder="Без НДС"
          />
        ...
        <InputWrapper>
          ...
          <InputAnchor
            ...
            onChange={(e) =>
              dispatch(sendContragentValues("agencyServices", e.target.value))
            }
            placeholder="Печать"
          />
        </InputWrapper>
